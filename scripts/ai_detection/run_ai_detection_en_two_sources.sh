...
LOG_ROOT="${LOG_ROOT:-$REPO/logs/ai-detection-selected-sources}"
...
GIT_PUSH_SUMMARIES="${GIT_PUSH_SUMMARIES:-0}"
RESUME="${RESUME:-1}"
...
download_required_data() {
  ...
  local lang_arg
  lang_arg="$(language_arg)"
  log_step "Downloading selected required HF dataset files into $RAW_ROOT for lang=$lang_arg"
  local downloads_file="$LOG_ROOT/required_hf_files_selected_sources.txt"
  run_python list-downloads --lang "$lang_arg" --include-human > "$downloads_file"
...
download_model() {
  ...
  if [[ -f "$local_dir/config.json" ]] && find "$local_dir" -maxdepth 2 -type f -name "*.safetensors" -print -quit | grep -q .; then
    echo "Model already present with safetensors: $repo_id -> $local_dir"
    return 0
  fi
  mkdir -p "$local_dir"
  echo "Downloading model without original/* or PyTorch .bin files: $repo_id -> $local_dir"
...
prepare_detector_models() {
  ...
  case "$detector" in
    binoculars)
      ...
      download_model "tiiuae/falcon-7b" "$MODELS_ROOT/binoculars/falcon-7b" "${COMMON_MODEL_EXCLUDES[@]}"
      download_model "tiiuae/falcon-7b-instruct" "$MODELS_ROOT/binoculars/falcon-7b-instruct" "${COMMON_MODEL_EXCLUDES[@]}"
      ;;
    fast_detect_gpt)
      log_step "Preparing Fast-DetectGPT models"
      download_model "meta-llama/Meta-Llama-3-8B" "$MODELS_ROOT/fast-detect-gpt/Meta-Llama-3-8B" "${COMMON_MODEL_EXCLUDES[@]}"
      download_model "meta-llama/Meta-Llama-3-8B-Instruct" "$MODELS_ROOT/fast-detect-gpt/Meta-Llama-3-8B-Instruct" "${COMMON_MODEL_EXCLUDES[@]}"
      ;;
    radar)
      log_step "Preparing RADAR model"
      download_model "TrustSafeAI/RADAR-Vicuna-7B" "$MODELS_ROOT/radar/RADAR-Vicuna-7B" "${COMMON_MODEL_EXCLUDES[@]}"
      ;;
    detect_gpt)
      log_step "Preparing DetectGPT models"
      download_model "EleutherAI/gpt-neo-2.7B" "$MODELS_ROOT/detect-gpt/gpt-neo-2.7B" "${COMMON_MODEL_EXCLUDES[@]}"
      download_model "google-t5/t5-large" "$MODELS_ROOT/detect-gpt/t5-mask" "${COMMON_MODEL_EXCLUDES[@]}"
      ;;
    ghostbuster)
...
cleanup_detector_models() {
  ...
  case "$detector" in
    binoculars|radar|detect_gpt)
      rm -rf -- "$MODELS_ROOT/$detector"
      ;;
    fast_detect_gpt)
      rm -rf -- "$MODELS_ROOT/fast-detect-gpt"
      ;;
    ghostbuster)
      ;;
  esac
...
check_row_counts() {
  local lang_arg
  lang_arg="$(language_arg)"
  log_step "Checking row counts for lang=$lang_arg"
  run_python check-row-counts \
    --raw-root "$RAW_ROOT" \
    --lang "$lang_arg" \
    --human-en "$HUMAN_EN" \
    --human-zh "$HUMAN_ZH" \
    --output-json "$WORK_DIR/selected_row_count_summary.json" \
    2>&1 | tee "$LOG_ROOT/selected-row-counts.log"
}

build_detector_inputs() {
  local lang_arg
  lang_arg="$(language_arg)"
  log_step "Building selected detector inputs under $WORK_DIR for lang=$lang_arg"
  run_python build-inputs \
    ...
    --human-en "$HUMAN_EN" \
    --human-zh "$HUMAN_ZH" \
    2>&1 | tee "$LOG_ROOT/selected-build-inputs.log"
}
...
run_detector_language() {
  ...
  log_step "Running detector=$detector language=$lang on GPUs ${GPUS[*]} with RESUME=$RESUME"
  ...
  run_python summarize --work-dir "$WORK_DIR" --lang "$lang" --detector "$detector" --merge-existing \
...
manifest_source_count() {
  local lang="$1"
  python - "$WORK_DIR/$lang/manifest.json" <<'PY'
import json
import sys
from pathlib import Path

manifest = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(len(manifest["sources"]))
PY
}

wait_for_detector_wave() {
  local detector="$1"
  local status=0
  local pid
  ...
  if (( status != 0 )); then
    echo "Detector failed: $detector. Check $LOG_ROOT/*-${detector}-*.log" >&2
    exit 1
  fi
  WAVE_PIDS=()
  WAVE_LABELS=()
...
run_detector_selected_languages() {
  local detector="$1"
  ...
  local task_langs=()
  local task_shards=()
  local task_num_shards=()
  local lang
  for lang in "${LANGS[@]}"; do
    local count
    count="$(manifest_source_count "$lang")"
    if (( count <= 0 )); then
      echo "No manifest sources for lang=$lang" >&2
      exit 1
    fi
    local shard
    for shard in $(seq 0 $((count - 1))); do
      task_langs+=("$lang")
      task_shards+=("$shard")
      task_num_shards+=("$count")
    done
  done

  log_step "Running detector=$detector with pooled language/source shards on GPUs ${GPUS[*]} and RESUME=$RESUME"

  WAVE_PIDS=()
  WAVE_LABELS=()
  local wave_slot=0
  local task_index
  for task_index in "${!task_langs[@]}"; do
    local task_lang="${task_langs[$task_index]}"
    local shard_index="${task_shards[$task_index]}"
    local num_shards="${task_num_shards[$task_index]}"
    local gpu="${GPUS[$wave_slot]}"
    local log_file="$LOG_ROOT/${task_lang}-${detector}-source-shard${shard_index}-of${num_shards}-gpu${gpu}.log"
    ...
    WAVE_PIDS+=("$!")
    WAVE_LABELS+=("${task_lang}/shard${shard_index}/gpu${gpu}")
    echo "[launch] detector=$detector lang=$task_lang shard=$shard_index/$num_shards gpu=$gpu log=$log_file"

    wave_slot=$((wave_slot + 1))
    if (( wave_slot >= ${#GPUS[@]} )); then
      wait_for_detector_wave "$detector"
      wave_slot=0
    fi
  done
...
if [[ "$CHECK_COUNTS_BEFORE_RUN" == "1" ]]; then
  check_row_counts || log_step "Row-count check reported mismatch/missing files; continuing"
fi

build_detector_inputs
git_commit_paths "Add selected AI detector input summaries" \
...
for detector in "${DETECTORS[@]}"; do
  ...
  run_detector_selected_languages "$detector"
  for lang in "${LANGS[@]}"; do
    upload_detector_outputs "$detector" "$lang"
    git_commit_paths "Add $lang $detector AI detector metrics" "$WORK_DIR/$lang/metrics/$detector"
  done
  cleanup_detector_models "$detector"
done

run_python combine-summaries --work-dir "$WORK_DIR" --lang "$(language_arg)" --merge-existing
git_commit_paths "Add combined AI detector metrics summary" "$WORK_DIR/metrics"

echo "Selected-source AI detector analysis completed."
