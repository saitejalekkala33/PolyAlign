  ...
  case "$detector" in
    ...
    detect_gpt)
      ...
      download_model "EleutherAI/gpt-neo-2.7B" "$MODELS_ROOT/detect-gpt/gpt-neo-2.7B" \
        --exclude "*.bin" --exclude "*.h5" --exclude "*.ot" --exclude "*.msgpack" \
        --exclude "flax_model.msgpack" --exclude "tf_model.h5" --exclude "rust_model.ot"
      download_model "google/t5-large" "$MODELS_ROOT/detect-gpt/t5-mask" \
        --exclude "*.bin" --exclude "*.h5" --exclude "*.ot" --exclude "*.msgpack" \
        --exclude "flax_model.msgpack" --exclude "tf_model.h5" --exclude "rust_model.ot"
      ;;
    ghostbuster)
...
build_aligned_inputs() {
  log_step "Building aligned detector inputs under $WORK_DIR"
...
upload_alignment_outputs() {
  if [[ "$UPLOAD_TO_HF" != "1" ]]; then
    log_step "Skipping alignment HF upload because UPLOAD_TO_HF=$UPLOAD_TO_HF"
    return 0
  fi
  for lang in zh en; do
    local hf_prefix
    [[ "$lang" == "zh" ]] && hf_prefix="chinese" || hf_prefix="english"
    log_step "Uploading alignment outputs for $lang to HF"
    hf upload "$HF_DATASET_REPO" "$WORK_DIR/$lang/alignment_summary.json" \
      "$hf_prefix/ai-analysis/alignment/alignment_summary.json" \
      --repo-type dataset \
      --commit-message "Upload $lang AI detector alignment summary"
    hf upload "$HF_DATASET_REPO" "$WORK_DIR/$lang/removed_indices.jsonl" \
      "$hf_prefix/ai-analysis/alignment/removed_indices.jsonl" \
      --repo-type dataset \
      --commit-message "Upload $lang AI detector removed indices"
  done
}
...
git_commit_alignment() {
  git_commit_paths "Add AI detector alignment summaries" \
    "$WORK_DIR/alignment_summary.json" \
    "$WORK_DIR/en/alignment_summary.json" \
    "$WORK_DIR/zh/alignment_summary.json"
}
...
for detector in binoculars fast_detect_gpt ghostbuster radar detect_gpt; do
